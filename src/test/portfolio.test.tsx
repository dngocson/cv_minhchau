...
describe("portfolio sections render with CV content", () => {
	it("renders the hero with name, role and a stat label", () => {
		render(<Hero />);
		expect(
			screen.getByRole("heading", { level: 1, name: "Nguyen Minh Chau" }),
		).toBeTruthy();
	...
	it("renders experience entries driven by dynamic i18n keys", () => {
		render(<Experience />);
		expect(
			screen.getByText("Tuong Ngoc International Investment Co., Ltd."),
		).toBeTruthy();
		// a bullet pulled via tList(returnObjects)
		expect(
			screen.getByText(
				/Managed and developed relationships with over 50 business clients/,
			),
		).toBeTruthy();
	});
